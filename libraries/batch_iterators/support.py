...
def pad_sent(sentence, max_length, pad_symbol, mask_current=False, padding_mode='both'):
    ...
    # padding both sides
        ...
        res = [pad_symbol] * (pad_number//2) + sentence + [pad_symbol] * (pad_number//2) # Python 3 integer division needed
        if mask_current:
            mask = [0]*(pad_number//2) + masked_sentence + [0]*(pad_number//2) # Python 3 integer division needed
        else:
            mask = [0]*(pad_number//2) + [1]*len(sentence) + [0]*(pad_number//2) # Python 3 integer division needed
        if pad_number % 2 == 1:
            res += [pad_symbol]
            mask += [0]
